...
export function findTodayJournalEntry(entries) {
  ...
  return entries.find((entry) => entry.journal_date === today) ?? null;
}
...
export function getNextUntitledEntryTitle(entries) {
...
export function getEntryTypeTag(entryType) {
  return entryType === "conversation" ? "#CONVERSATION" : "#NOTE";
}

export function buildMarkdownExport({ title, body, occurredAt }) {
  const cleanTitle = title?.trim() || "Untitled";
  const cleanBody = body?.trimEnd() || "";
  const dateLine = occurredAt ? `Date: ${getLocalDayKey(new Date(occurredAt))}\n\n` : "";

  return `# ${cleanTitle}\n\n${dateLine}${cleanBody}\n`;
}

export function getMarkdownExportFilename({ title, occurredAt }) {
  const datePrefix = occurredAt ? `${getLocalDayKey(new Date(occurredAt))}-` : "";
  const cleanTitle = title?.trim() || "untitled";
