...
import {
  ...
  deleteEntriesInputSchema,
  idSchema,
  updateEntryInputSchema,
} from "./contracts.js";
import { deleteEntryChunks, reindexEntryChunks } from "./chunks.js";
...
export async function createManualEntry(input) {
  ...
  await reindexEntryChunks(entry);
...
export async function deleteEntries(input) {
  const { ids } = deleteEntriesInputSchema.parse(input);

  return getDb().transaction(async (tx) => {
    await tx.delete(chunks).where(inArray(chunks.entryId, ids));
