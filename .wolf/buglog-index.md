...
| innerscript-bug-014 | 2026-06-05 | tests, dependencies, storybook, vitest, postgres, sandbox | `node_modules`; `vitest.config.js`; `tests/integration/setup.js` | Focused reflection tests initially failed because unit Vitest could not resolve Storybook/Vitest browser packages and integration tests could not connect to localhost Postgres in the sandbox. | Ran `npm install`, reran unit tests successfully, and reran integration with platform escalation plus `TEST_DATABASE_URL`. |
